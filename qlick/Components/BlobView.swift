...
struct BlobView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            withAnimation(.linear(duration: 15).repeatCount(4, autoreverses: true)) {
                appear = true
            }
        }
    }
//            .foregroundStyle(.linearGradient(colors: [.pink, .blue], startPoint: .topLeading, endPoint: .bottomTrailing))
    ...
    func path(in rect: CGRect, x: Double, x2: Double) -> Path {
        ...
        path.addCurve(to: CGPoint(x: 0.6355*width, y: height), control1: CGPoint(x: 0.92554*width*x2, y: 0.77749*height*x2), control2: CGPoint(x: 0.91864*width*x2, y: height))
